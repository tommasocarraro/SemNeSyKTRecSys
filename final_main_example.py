...
from src.data_preprocessing.Preprocess_Strategy import SplitStrategy, LeaveLastOut
...
from src.source_pretrain.model_configs import get_config
from src.source_pretrain.trainer import MfTrainer
from src.target.trainer import LTNTrainer, LTNRegTrainer
...
config = get_config(dataset_name="movies", kind="train")

dataset = process_source_target(
    split_strategy=SplitStrategy(
        src_split_strategy=LeaveLastOut(seed=config.seed), tgt_split_strategy=LeaveLastOut(seed=config.seed)
    ),
...
trainer_src = MfTrainer(mf_model_src, torch.optim.AdamW(params=mf_model_src.parameters(),
...
trainer_tgt = LTNRegTrainer(mf_model_tgt,
                            torch.optim.AdamW(params=mf_model_tgt.parameters(), lr=0.001, weight_decay=0.00001),
                            get_reg_axiom_data(dataset.src_ui_matrix,
                                               dataset.tgt_ui_matrix,
                                               100,  #dataset.n_sh_users,
                                               dataset.sim_matrix,
                                               generate_pre_trained_src_matrix(mf_model_src,
                                                                               Path(
                                                                                   "./source_models/best_src_movies.pth"),
                                                                               100,  # dataset.n_sh_users,
                                                                               k=20,
                                                                               batch_size=512)))
...
trainer_tgt.train(
    ...
    early_stopping_criterion="val_metric",
    save_paths=(
        Path("./source_models/checkpoint_src_books.pth"),
        Path("./source_models/best_src_books.pth"),
    )
)
