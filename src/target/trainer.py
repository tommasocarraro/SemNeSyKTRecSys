...
class LTNTrainer(MfTrainer):
    ...
    def __init__(self, mf_model, optimizer, p_forall=2, wandb_train=False):
        """
        ...
        :param p_forall: hyper-parameter p for universal quantifier aggregator
        :param wandb_train: whether the training information has to be logged on WandB or not
        """
        super(LTNTrainer, self).__init__(mf_model, optimizer, LTNLoss(), wandb_train)
    ...
    def train_epoch(self, train_loader, epoch=None):
        ...
        for batch_idx, (u, i_pos, i_neg) in enumerate(train_loader):
            ...
            train_sat = self.Forall(ltn.diag(user, item_pos, item_neg),
                                    self.loss(self.Score(user, item_pos), self.Score(user, item_neg)),
                                    p=self.p_forall).value
...
class LTNRegTrainer(MfTrainer):
    ...
    def __init__(self, mf_model, optimizer, processed_interactions, p_sat_agg=2, p_forall=2, wandb_train=False):
        """
        Constructor of the trainer for the basic Logic Tensor Network model.
        ...
        to be performed
        :param p_sat_agg: hyper-parameter p for sat aggregator
        :param p_forall: hyper-parameter p for universal quantifier aggregator
        :param wandb_train: whether the training information has to be logged on WandB or not
        """
        super(LTNRegTrainer, self).__init__(mf_model, optimizer, LTNLoss(), wandb_train)
    ...
    def train_epoch(self, train_loader, epoch=None):
        ...
        for batch_idx, (u, i_pos, i_neg) in enumerate(train_loader):
            ...
            item_pos = ltn.Variable("item_pos", i_pos, add_batch_dim=False)
            item_neg = ltn.Variable("item_neg", i_neg, add_batch_dim=False)
            axiom1 = self.Forall(ltn.diag(user, item_pos, item_neg),
                                 self.loss(self.Score(user, item_pos), self.Score(user, item_neg)),
                                 p=self.p_forall)
            ...
            neg_score = ltn.Variable("neg_score", torch.tensor([0.] * u.shape[0]), add_batch_dim=False)
            # define variables
            ...
            # define axiom 2
            axiom2 = self.Forall(ltn.diag(reg_user, reg_item, neg_score),
