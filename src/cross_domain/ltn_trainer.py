...
class LTNRegTrainer(Trainer):
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int):
        ...
        for batch_idx, (u, i_pos, i_neg, sampled_sh_users, sampled_pos_items, sampled_neg_items) in enumerate(
            tqdm(train_loader, desc=f"Training epoch {epoch}", dynamic_ncols=True)
        ):
            self.optimizer.zero_grad()
            ...
            # axiom 2
            reg_user = ltn.Variable("reg_user", sampled_sh_users, add_batch_dim=False)
            reg_pos_item = ltn.Variable("reg_pos_item", sampled_pos_items, add_batch_dim=False)
            reg_neg_item = ltn.Variable("reg_neg_item", sampled_neg_items, add_batch_dim=False)
            axiom2 = self.Forall(
