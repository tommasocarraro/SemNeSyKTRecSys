CREATE FUNCTION get_book_info_by_isbn(isbn_param TEXT) RETURNS TABLE (
  title VARCHAR(200),
  person TEXT [],
  year CHAR(4)
) AS $ $ DECLARE work_keys_var TEXT [];
...
FROM
  (
    SELECT
      CASE
        WHEN work_title_var IS NULL THEN edition_title_var
        ELSE work_title_var
      END AS book_title2,
    ...
    FROM
      authors a
    WHERE
      key = ANY (COALESCE(author_keys_var, '{}' :: TEXT []))
  ) t
  CROSS JOIN LATERAL UNNEST(author_keys_var) WITH ORDINALITY AS u(key, ord)
...
CREATE FUNCTION get_book_info_by_title(title_param VARCHAR(200)) RETURNS TABLE (
  person TEXT,
  year CHAR(4)
) AS $ $ DECLARE author_keys_var TEXT [];

author_names_var TEXT;

year_var CHAR(4);

BEGIN WITH distinct_title_authors AS (
  SELECT
    DISTINCT ON (e.title_query, e.authors) e.authors,
