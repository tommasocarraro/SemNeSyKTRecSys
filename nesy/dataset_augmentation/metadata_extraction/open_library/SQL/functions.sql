...
BEGIN
    ...
    IF work_keys_var IS NOT NULL THEN
        SELECT w.title, w.authors INTO work_title_var, author_keys_var
        FROM works w
    ...
    FROM (
        SELECT
            CASE WHEN work_title_var IS NULL THEN edition_title_var ELSE work_title_var END AS book_title2,
            min_year_var AS year,
            a.key
        FROM authors a
        WHERE key = ANY(COALESCE(author_keys_var, '{}'::TEXT[]))
    ) t
...
BEGIN
    ...
    SELECT DISTINCT ON (e.title_query, e.authors)
        e.authors,
